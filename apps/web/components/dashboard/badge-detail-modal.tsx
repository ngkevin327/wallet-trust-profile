...
export function BadgeDetailModal({ badge, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      <div role="dialog" aria-modal="true" className="ui-card-elevated w-full max-w-sm shadow-glow">
        <h3 className="section-title">{badge.title}</h3>
        ...
        <button type="button" onClick={onClose} className="ui-btn ui-btn-secondary mt-6 w-full">
          Close
        </button>
      </div>
    </div>
